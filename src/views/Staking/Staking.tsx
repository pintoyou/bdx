  ...
  return (
    <Switch>
      <Page>
        ...
        <StyledCardHeader>
            <Label text={`Welcome to GamePool, stake Game to earn Game.`}/>
        </StyledCardHeader>
        <div style={{fontWeight: 'bold', fontSize: 22, color: '#ffffff'}}>
            Game Currently has <span style={{color: '#4caf50', fontSize: 30}}>{parseFloat(getBalanceNumber(new BigNumber(totalSupply)).toFixed(2)).toLocaleString('en-US')}</span> Game Staked
        </div>
        <Spacer size="md" />
        {!!account ? (
          <>
