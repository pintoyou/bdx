  ...
  return (
    <StyledWrapper>
      <Card>
        <CardContent>
          <StyledBalances>
            <StyledBalance>
              {/* <SushiIcon /> */}
              <img src={Bscx} alt="Game Balance"/>
              <Spacer />
              <div style={{ flex: 1 }}>
                <Label text="Your Game Balance" />
                <Value
        ...
        <Footnote>
          Total locked amount
          <FootnoteValue>
            {totalUserLocked ? `${parseFloat(getBalanceNumber(totalUserLocked).toFixed(2)).toLocaleString('en-US')} Game` : '~'}
          </FootnoteValue>
        </Footnote>
        <Spacer size="sm" />
      </Card>
      ...
      <Card>
        <CardContent>
          <StyledBalance>
            <img height="50px" src={Bscxs} alt="Total Game Supply"/>
            <Spacer />
            <div style={{ flex: 1 }}>
              <Label text="Game Circulating Supply" />
              <Value
        ...
        <Footnote>
          Total farmed amount
          <FootnoteValue>
            {currentFarmed ? `${parseFloat(getBalanceNumber(currentFarmed).toFixed(2)).toLocaleString('en-US')} Game` : '~'}
          </FootnoteValue>
        </Footnote>
        <Footnote>
          Total contract locked amount
          <FootnoteValue>
            {totalLocked ? `${parseFloat(getBalanceNumber(totalLocked).toFixed(2)).toLocaleString('en-US')} Game` : '~'}
          </FootnoteValue>
        </Footnote>
        <Spacer size="sm" />
      </Card>
